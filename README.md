...
2.  **Ensure you have Python 3.x and `uv` installed.** If not, you can install `uv` via `pip`:
    ```bash
    pip install uv
    ```
...
3.  **Install dependencies:**
    Navigate to the `todo-console-app` directory and install the project dependencies using `uv`:
    ```bash
    cd todo-app
    uv sync
    ```
...
Once the setup is complete, you can run the todo application from the `todo-app` directory:
